...
export default function LyricsBar() {
  ...
  if (!hide || currentLyric === '') return null;

  return (
    <div
      key={currentLyric}
      role="status"
      className="absolute bottom-full left-1/2 mb-2 max-w-[min(80vw,560px)] -translate-x-1/2 truncate rounded-full bg-(--md-sys-color-surface-container-high) px-4 py-1.5 text-sm shadow-lg animate-[fade-in_0.3s_ease-out]"
    >
      {currentLyric}
    </div>
  );
